 ...
 *           (C) 2024  rusq - porting to macOS and FreeBSD
 *
...
// Maximum number of files in a group file: (taken from the original
// "kextract.c")
...
int main(int ArgC, char *ArgV[]) {
	...
	for (Ind = 0; Ind < NbFiles; Ind++) {
		...
		FilesProperties[Ind].Name[12] =
			'\0'; // "close" the name manually if it was too long (12 chars)

		// Extract the length of the file
		fread(Buffer, 1, 4, GroupFile);
		FilesProperties[Ind].Length =
	...
	for (Ind = 0; Ind < NbFiles; Ind++) {
		...
		} // for (Ind2 = 0; Ind2 < FilesProperties[Ind].Length; Ind2 += sizeof
		  // (Buffer))
		printf("done\n");
		fclose(CrtFile);
...
static void MarkFiles(const char *FileSpec) {
	...
	// Put the FileSpec in LocalFileSpec with a format on 12 characters
	// ("<prefix>_SPACES_.<suffix>")
	Ind2 = 0;
	...
	for (Ind = 0; Ind < NbFiles; Ind++) {
		// Put the FileProperties[Ind].Name in LocalFileName with the same format on
		// 12 characters
		Ind2 = 0;
...
static bool ExtractPictures(void) {
	...
	for (Ind = 0; Ind < NbTiles; Ind++) {
		sprintf(ImageFileName, "%03d-%c%c%c.tga", Ind, ArtFileName[5], ArtFileName[6],
				ArtFileName[7]);
...
static bool SpawnTGA(unsigned int TileInd, const char *PictureName) {
	...
	unsigned char TgaHeader[] = {
		...
		0x00, 0x00, 0x00, 0x01, 0x18, // Color Map Specification (256 colors of 24 bits, starting
									  // from 0)
		0x00, 0x00, 0x00,
		0x00, // Origin of Image (0, 0)
		0x00, 0x00, 0x00,
		0x00, // Size of Image (TO BE FILLED. Bytes 12-13 and 14-15.
			  // little-endian)
